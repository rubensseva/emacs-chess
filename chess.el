...
(defun query-move (board)
    (let ((piece-regex "^[a-h][1-9]$") (text-to-insert (read-string "piece to move?")))
	(if (string-match piece-regex text-to-insert)
	      (let ((indexes (get-index text-to-insert)))
		(let ((piece (get-piece indexes board)))
		  (message "returning %s" indexes)
		  (list indexes piece)
		  )
	    )
...
(defun chess ()
  ...
  (let ((buffer-name "chess-buffer-foo") (board (create-board)))
    ...
    (print-board board)
    (let ((curr_player 1))
      (let ((curr_piece_and_index (query-move board)))
	(let ((mov_piece_and_index (query-move board)))
	  (message "printing")
	  (message "just testing %s" (query-move board))
	  (message "curr piece: %s" curr_piece_and_index)
	  (message "mov piece: %s" mov_piece_and_index)
